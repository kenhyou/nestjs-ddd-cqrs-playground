...
- **Authoritative reference**: `examples/order-management-basic/`, `examples/order-management-intermediate/` (git-tracked, committed). When in doubt, match it.
- For **layering rules** and the architecture summary, see `CLAUDE.md` (authoritative — not duplicated here).
- For the **8-phase curriculum and per-layer tests**, see `PLAN.md` (authoritative).
- This file covers **file/folder naming, method order, class skeletons, in-class naming rules, and testing conventions**.
...
> Domain events are recorded by aggregates inside `domain/` (the business fact). The `@EventsHandler` classes that _react_ to them live in `application/events/handlers/` — do not put handlers in `domain/events/`.
...
- **Existence queries**: `exists()`, never `isExists` / `doesExist`.
- **State/property booleans**: `is~` only for adjective-like states (`isActive`, `isExpired`, `isEmpty`).
- **Verb booleans**: the verb itself — `overlaps`, `contains`, `equals`.

---

## Method order

Group methods by category; one blank line between categories.

**Aggregate Roots / Entities** (`domain/models/`):

1. Field declaration via `private constructor` parameter properties.
2. Static factories — `static create()` (new, `generate()`s id), `static reconstitute()` (from DB, no events).
3. Business behavior — `confirm()`, `cancel()`, state-transition methods (guard → mutate → record).
4. Derived calculations / queries — `getTotalPrice()`, `isActive()`.
5. Getters.

**Value Objects** (`domain/vo/`):

1. `private constructor`
2. `static create()` with validation (plus `static generate()` for ID VOs)
3. `getValue()` / `getX()` getters
4. `equals()`
5. Domain methods — `overlaps()`, `contains()`.
...
- **Assert on error _type_, not message.** Until Phase 7's typed exceptions, use bare `.toThrow()` — never couple to message copy.
