...
@Injectable()
export class RefundIssuedHandler extends MessageHandler {
  readonly messageType = 'RefundIssued';
  ...
  async handle(message: InboundMessage): Promise<void> {
    ...
    saga.onRefundIssued();
    order.recordPaymentStatus('REFUNDED');
    // Guarded: if the order already shipped (rare timeout-after-dispatch window),
    // it cannot be cancelled (D6a) — the refund still stands.
